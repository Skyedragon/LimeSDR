...
class LimeSDR:
  ...
  def __init__(self, meas_time, rx_bw, fs, channel): #rx_bw - bandwidth, fs - sampling rate
    self.channel = channel
    use_agc = True          # Use or don't use the AGC
    self.freq = 0.520e9            # LO tuning frequency in Hz
    self.timeout_us = int(5e6)
    self.N = int(fs * meas_time)               # Number of complex samples per transfer
    self.rx_bits = 12            # The AIR-T's ADC is 16 bits
    RX1 = 0             # RX1 = 0, RX2 = 1
    RX2 = 1
    self.sdr = SoapySDR.Device(dict(driver="lime")) # Create AIR-T instance
    self.sdr.setSampleRate(SOAPY_SDR_RX, 0, fs)          # Set sample rate
    self.sdr.setGainMode(SOAPY_SDR_RX, 0, use_agc)       # Set the gain mode
    self.sdr.setFrequency(SOAPY_SDR_RX, 0, self.freq)         # Tune the LO
...
if __name__ == "__main__":
  ...
  if args.time and args.samprate and args.bw and args.filename and args.channel:
    ...
    if ( len(args.channel) == len(args.filename[0]) and len(args.channel) == 1 ):
      ...
      Lime = LimeSDR(args.time, args.bw, args.samprate, args.channel)
      Lime.get_signal()
